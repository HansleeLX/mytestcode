(function($) {
    ...
    function OrgChart($container, opts){
        ...
        this.startEdit = function(id){
            ...
            var commitChange = function(){
                var h2Element = $('<h2 class="spec_h2">'+nodes[id].data.name+'</h2>');
			
				if(nodes[id].data.parent == '1')
				{
					var positioninfo = id+', '+nodes[id].data.name+', '+nodes[id].data.parent;
					var positionnum = 'position'+id;
					document.getElementById(positionnum).value = positioninfo;
					//alert(document.getElementById(positionnum).value);
					//alert(id+', '+nodes[id].data.name+', '+nodes[id].data.parent);
				}
				if(nodes[id].data.parent == '2')
				{
					var positioninfo = id+', '+nodes[id].data.name+', '+nodes[id].data.parent;
					var positionnum = 'position'+id;
					document.getElementById(positionnum).value = positioninfo;
					//alert(data.parent+', '+totallowerlv);
					//alert(id+', '+nodes[id].data.name+', '+nodes[id].data.parent);
					//alert(document.getElementById(positionnum).value);
				}
				
				//alert(nodes[id].data.parent);
                if(opts.allowEdit){
        ...
        this.addNode = function(data){
			...
			//in this way, we conclute the node
			//alert(data.parent);
			
			//this show the total number of nodes
			//alert(data.id);
            self.draw();
        ...
        for(var i in nodes){
            if(nodes[i].data.parent == 0){
				...
				//this time record the number of children for each node;
				//alert(nodes[nodes[i].data.parent].children.length);
            }
        }
    ...
    function Node(data){
        ...
        this.render = function(opts){
            ...
            mainTable = "<div class='table-responsive'><table cellpadding='0' cellspacing='0' border='0'>";
        ...
        this.formatNode = function(opts){
            ...
            if(typeof data.name !== 'undefined'){
                nameString = '<h2 class="spec_h2">'+self.data.name+'</h2>';
            }
